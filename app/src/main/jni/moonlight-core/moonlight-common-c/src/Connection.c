...
static void ClInternalConnectionTerminated(int errorCode) {
    ...
    err = PltCreateThread("AsyncTerm", terminationCallbackThreadFunc, NULL,
                          &terminationCallbackThread);
    if (err != 0) {
...
int LiStartConnection(PSERVER_INFORMATION serverInfo, PSTREAM_CONFIGURATION streamConfig,
    ...
    if (RtspPortNumber != 48010) {
        ...
        err = resolveHostName(serverInfo->address, AF_UNSPEC, RtspPortNumber, &RemoteAddr,
                              &AddrLen);
        if (err != 0) {
