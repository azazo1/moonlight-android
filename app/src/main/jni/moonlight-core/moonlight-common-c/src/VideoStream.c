...
static void VideoReceiveThreadProc(void *context) {
    ...
    minSize = sizeof(RTP_PACKET) +
              ((EncryptionFeaturesEnabled & SS_ENC_VIDEO) ? sizeof(ENC_VIDEO_HEADER) : 0);
    ...
    while (!PltIsThreadInterrupted(&receiveThread)) {
        ...
        if (encrypted) {
            ...
            if (!PltDecryptMessage(decryptionCtx, ALGORITHM_AES_GCM, 0,
                                   ...
                                   ((unsigned char *) (encHeader + 1)), err -
                                                                        sizeof(ENC_VIDEO_HEADER), // The ciphertext is after the header
        ...
        queueStatus = RtpvAddPacket(&rtpQueue, packet, err,
                                    (PRTPV_QUEUE_ENTRY) &buffer[decryptedSize]);
...
void stopVideoStream(void) {
    ...
    if ((VideoCallbacks.capabilities & (CAPABILITY_DIRECT_SUBMIT | CAPABILITY_PULL_RENDERER)) ==
        0) {
